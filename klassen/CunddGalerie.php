...
class CunddGalerie extends CunddAttribute {
	...
	function createRandomStepIntoLink($width = NULL, $height = NULL){
	...
	function stepIntoLink($pointerToImage = NULL){
		...
		// Link ausgeben
		echo CunddTemplate::inhalte_einrichten($wert, 4, $this->classPrefix.'_step_into_'.$this->classPrefix.'_link', 'output');
	}
		...
		foreach($newImageArray as $key => &$image){
			...
			switch($filedata['files_type']) {
				...
				case 'image/pjpeg': //wegen IE
					$saved *= imagejpeg($image, $uploadTempPath,100);
					break;
				case 'image/png':
